...
async function addProduct() {
    ...
    if (productName && initialValue >= 0 && usedValue >= 0 && initialValue >= usedValue) {
        ...
        const excelDate = `${day}${month}${year}`; // DDMMYYYY
        const descriptionHdr = 'Stock Issue';
        const itemCode = productName.split(' (')[0]; // Extract code before "(", e.g., "Tepung" from "Tepung (Flour)"
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert('Error saving data to server: ' + response.statusText);
            }
        } catch (error) {
    ...
    request.onload = function () {
        if (request.status === 200) {
            ...
            headers.forEach((header, colIndex) => {
                ...
                ws[cellAddress] = {
                    v: header + (header.includes('(') ? '' : ''), // Add original format if needed
                    t: 's',
